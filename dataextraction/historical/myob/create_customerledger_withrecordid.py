...
def update_customer_ledger(ledger_df, cust_df):
    ...
    for _, row in cust_df.iterrows():
        ...
        full_name_key = norm(f"{last}, {first}").strip(",")
    ...
    for _, row in ledger_df.iterrows():
        ...
        try:
            (
                date,invoiceno,customerPONo,customer_name,Amount,AmountDue,Status
            ) = row
        except ValueError:
            continue
        ...
        cleaned_rows.append([
            date,invoiceno,customerPONo,
            customer_name,
            Amount,AmountDue,Status,
            recordid
        ])
