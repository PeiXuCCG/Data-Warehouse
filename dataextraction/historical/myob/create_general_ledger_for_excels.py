...
company = "willaid"
GL_PATH = f"{company}/glentry/sources/excel/*.xlsx"
OUTPUT_FILE = f"{company}/glentry/cleaned.csv"

HEADINGS = [
    "date","referencenumber","transactiontype","transactiondescription",
    "open","debit","credit","netactivity","balance","taxamount"
]

def load_excels(path):
    files = glob.glob(path)
    dfs = []
    for f in files:
        df = pd.read_excel(f, header=None, dtype=str, keep_default_na=False)
        df["filename"] = f
        dfs.append(df)
    return pd.concat(dfs, ignore_index=True)

gl = load_excels(GL_PATH)
...
for _, row in gl.iterrows():
    ...
    try:
        pd.to_datetime(first, dayfirst=True)
        is_transaction = True
    except:
